...
export const url = {
  ...
  view(params: Partial<Pick<UrlParam, 'viewId'>> & Pick<UrlParam, 'organizationId' | 'projectId'>) {
    if (!params.viewId) return url.project(params) + '/views';
    return url.project(params) + '/views/' + params.viewId;
  },
  form(params: Partial<Pick<UrlParam, 'formId'>> & Pick<UrlParam, 'organizationId' | 'projectId'>) {
    if (!params.formId) return url.project(params) + '/forms';
    return url.project(params) + '/forms/' + params.formId;
  },
  workflow(
    params: Partial<Pick<UrlParam, 'workflowId'>> & Pick<UrlParam, 'organizationId' | 'projectId'>
  ) {
    if (!params.workflowId) return url.project(params) + '/workflows';
    return url.project(params) + '/workflows/' + params.workflowId;
  },
  table(
    params: Partial<Pick<UrlParam, 'tableId'>> & Pick<UrlParam, 'organizationId' | 'projectId'>
  ) {
  ...
  reorderVieldAfter(
    params: Partial<Pick<UrlParam, 'fieldId'>> &
      Pick<UrlParam, 'organizationId' | 'projectId' | 'viewId'> & { afterFieldId: string }
  ) {
    const viewUrl = url.view(params);
    const fieldUrl = viewUrl + '/fields';
  ...
  addRowRelation(
    params: Pick<UrlParam, 'organizationId' | 'projectId' | 'tableId' | 'fieldId' | 'rowId'>
  ) {
    const tableUrl = url.table(params);
  ...
  removeRowRelation(
    params: Pick<
      UrlParam,
      'organizationId' | 'projectId' | 'tableId' | 'fieldId' | 'rowId' | 'refRowId'
    >
  ) {
    const tableUrl = url.table(params);
    return (
      tableUrl + '/rows/' + params.rowId + '/relation/' + params.fieldId + '/' + params.refRowId
    );
  },
  ...
  member(
    params: Partial<Pick<UrlParam, 'memberId'>> & Pick<UrlParam, 'organizationId' | 'projectId'>
  ) {
    return !params.memberId
      ? url.project(params) + '/members'
      : url.project(params) + '/members/' + params.memberId;
  },
  role(params: Partial<Pick<UrlParam, 'roleId'>> & Pick<UrlParam, 'organizationId' | 'projectId'>) {
