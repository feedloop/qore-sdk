...
describe("Qore Project", () => {
  ...
  describe("Auth", () => {
    it("should login with user token", async () => {
      ...
      expectAsync(
        client.auth.signInWithUserToken(mockUserToken)
      ).toBeResolved();
    });
  ...
  describe("Roles", () => {
    it("should create role", async () => {
      ...
      expectAsync(
        client.createRole({
          name: "editor",
          permissions: ["tableWrite", "tableExecute", "tableRead", "view"]
        })
  ...
  describe("Table", () => {
    ...
    describe("Rows", () => {
      ...
      it("should update row field", async () => {
        ...
        const row = await node.row(mockFirstRowId);
        expectAsync(
          row.col<"text">("nameTxtField").update("new-rama")
        ).toBeResolved();
      });
      it("should update row field", async () => {
        const client = getQoreProject();
      ...
      it("should add row field relation", async () => {
        ...
        const row = await node.row(mockFirstRowId);
        expectAsync(
          row.col<"relation">("testName").add("row-id-3")
        ).toBeResolved();
      });
      it("should delete row field relation", async () => {
        ...
        const row = await node.row(mockFirstRowId);
        expectAsync(
          row.col<"relation">("testName").remove("row-id-3")
        ).toBeResolved();
      });
    });
  });
    ...
    it("should create view", async () => {
      ...
      expectAsync(
        client.createView({
          ...
          vields: ["testAction"],
          tableId: mockTable,
          filters: ["status == 'active'", "expirationDate > now()"],
          sorts: [
            {
              by: "expirationDate",
              order: "desc"
